...
Window-level scores are derived from per-segment scores — they are aggregations, not independent computations. The aggregation method varies by dimension (sections 3–6) because the dimensions have different semantics. Coherence is about relationships between segments, so the window score reflects pairwise structure. Density is about individual segments, so the window score is a weighted average. The details are in each dimension's section.
...
- **Relevance** aggregates to the group level for eviction purposes. The group's relevance is the aggregate of its members' relevance (section 9.4), because groups are evicted atomically — the eviction advisor needs a single relevance score per eviction candidate.
...
```
protectionAdjust(baseRelevance, protection):
  pinned      -> 1.0                          (always 1.0, ignores base score)
  seed        -> max(baseRelevance, 0.3)      (floor at 0.3)
  priority(n) -> baseRelevance                (pass-through)
  default     -> baseRelevance                (pass-through)
```
...
Recency is relative to the assessment timestamp, not the system clock. The assessment timestamp is the report's `timestamp` field, established at the start of each `assess()` call.

```
age(i) = assessmentTimestamp - segment[i].createdAt (or updatedAt, whichever is more recent)
recency(i) = 1.0 - (age(i) / maxAge)
```

Where:
- `assessmentTimestamp` = the timestamp captured when `assess()` begins (equal to the report's `timestamp` field)
- `age(i)` = `assessmentTimestamp` - `segment[i].createdAt` (or `updatedAt`, whichever is more recent)
- `maxAge` = age of the oldest active segment (relative to `assessmentTimestamp`)
...
Each segment's relevance score combines content, metadata, and temporal signals. The base weighted sum is computed first, then protection is applied as a post-hoc clamp/floor:

```
baseRelevance(i) = contentRelevance(i)  * w_content   +
                   keywordBoost(i)      * w_keyword    +
                   originMatch(i)       * w_origin     +
                   recencyWeight(i)     * w_recency    +
                   importanceWeight(i)  * w_importance

relevance(i) = protectionAdjust(baseRelevance(i), segment[i].protection)
```
...
| `w_keyword` | 0.10 | Keyword overlap provides precision |
| `w_origin` | 0.10 | Origin is a coarse but reliable indicator |
| `w_recency` | 0.20 | Recency is a temporal relevance signal |
| `w_importance` | 0.15 | Importance is an explicit caller signal of value |

The weights sum to 1.0. Protection is then applied via `protectionAdjust` as a post-hoc adjustment — pinned segments are set to 1.0, seeds are floored at 0.3, others pass through unmodified.
...
| `tokenizer` | TokenizerMetadata | Active tokenizer name, accuracy, error bound, model family (cl-spec-006 section 2.2) |
...
| `continuity` | ContinuitySummary | Summary of eviction/compaction/restoration history (section 9.5) |
| `baselineEstablished` | boolean | Whether a baseline has been captured (section 7.2) |
| `rawScores` | WindowScores | Raw (unnormalized) quality dimension scores. Null before baseline. |
| `embeddingMode` | string | "embeddings" or "trigrams" — similarity mode used for this report |
| `patterns` | DetectionResult | Active degradation patterns (cl-spec-003 section 2.3) |
| `task` | TaskSummary | Current task state summary. Null if no task set (cl-spec-004) |
...
| Field | Type | Description |
|-------|------|-------------|
| `coherence` | number (0.0–1.0) \| null | Normalized window coherence (section 3.7 / 7.3) |
| `density` | number (0.0–1.0) \| null | Normalized window density (section 4.6 / 7.3) |
| `relevance` | number (0.0–1.0) \| null | Normalized window relevance (section 5.7 / 7.3) |
| `continuity` | number (0.0–1.0) \| null | Window continuity (section 6.7 — not normalized, already absolute) |
| `raw.coherence` | number \| null | Pre-normalization coherence score |
| `raw.density` | number \| null | Pre-normalization density score |
| `raw.relevance` | number \| null | Pre-normalization relevance score |
| `raw.continuity` | number \| null | Same as normalized (continuity has no baseline normalization) |

In the empty-window case (zero active segments), all WindowScores fields are `null` (see invariant 4).
...
| `segmentCountDelta` | number | Change in segment count (positive = growth, negative = shrinkage) |
...
2. **Deterministic scoring.** The same window state (same segments, same content, same metadata, same task descriptor, same baseline) produces the same scores. Scores do not depend on wall-clock time, report generation order, or external state. Recency (section 5.4) is computed relative to the assessment timestamp (the report's `timestamp` field, captured at the start of `assess()`), not the system clock — given the same assessment timestamp and segment metadata, recency scores are deterministic.
